...
- 所有叙事状态都保存在当前浏览器中，并可通过“存档胶囊”导出或恢复，不需要账号或后端。
...
游戏状态只保存在当前浏览器的 `localStorage`；导入功能只读取玩家主动选择的存档胶囊文件。项目不会访问摄像头、麦克风、浏览历史、联系人、账户信息或其他敏感数据。需要注意力与页面焦点的玩法只使用游戏区域内的指针位置，以及标准的页面可见性 / 焦点事件。
